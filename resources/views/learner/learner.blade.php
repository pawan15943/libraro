...
<div class="row">
    <div class="col-lg-12 text-end">
        <a href="{{ route('learners.export-csv') }}" class="btn btn-primary export" data-bs-toggle="tooltip"
            data-bs-placement="bottom" data-bs-title="Filter" id="filter"><i class="fa-solid fa-filter"></i></a>
        ...
        <a href="{{ route('learners.export-csv') }}" class="btn btn-primary export" data-bs-toggle="tooltip"
            data-bs-placement="bottom" data-bs-title="Export Learners Data to CSV"><i
        ...
        <a href="{{ route('library.upload.form') }}" class="btn btn-primary export bg-4" data-bs-toggle="tooltip"
            data-bs-placement="bottom" data-bs-title="Import Learners Data to Portal"><i
            ...
            <form action="{{ route('learners') }}" method="GET">
                    ...
                    <div class="col-lg-2">
                        <select name="plan_id" id="plan_id2" class="form-select">
                            ...
                            <option value="{{ $plan->id }}"
                                {{ request()->get('plan_id') == $plan->id ? 'selected' : '' }}>
                                {{ $plan->name }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="col-lg-3">
                    <select name="seat_no" id="seat_search" class="form-select">
                        ...
                        @for($seatNo = 1; $seatNo <= $totalSeats; $seatNo++) <option value="{{ $seatNo }}"
                            {{ request()->get('seat_no') == $seatNo ? 'selected' : '' }}>
...
<div class="col-lg-12 mb-4" id="countsContainer">
    <div class="records">
        ...
        <p class="mb-2 text-dark"><b>Total Seats : {{$total_seats}} | Available Seats : {{$availble_seats}} | Booked
                Seats: {{$booked_seats}} | General Seats: {{$genral_seat ?? 0}}</b></p>

        <span class="text-success">Total Available Slots ({{$availble_seats}})</span> <span class="text-success">Total
            Booked Slots ({{$active_seat_count}})</span> <span class="text-success">Total General Booked Slots
            ({{$genral_seat}})</span><span class="text-danger">Total Expired Slots({{$expired_seat}})</span> <span
            class="text-danger">Extended Slots({{$extended_seats}})</span>
    ...
    <div class="col-lg-12">
        <div class="seat-info bg-white">
            <div class="seat-no">
                ...
                <span class="extended"> Closed Seat on
                    {{ $operationDate ? date('j M Y', strtotime($operationDate)) : '' }}</span>
                @elseif($operation == 'deleteSeat' && $value->deleted_at !=null)
                <span class="extended"> Deleted Seat on
                    {{ $operationDate ? date('j M Y', strtotime($operationDate)) : '' }}</span>
                @else
            ...
            <div class="seat-actions">
                <ul>
                    <li><a href="https://wa.me/+91{{ $value->mobile }}?text={{ urlencode('Hello! Please click the link below to access the Attendance App and mark your attendance securely. ' . route('qr.attendance.link')) }}"
                            target="_blank" class="w-auto px-2">
                            <i class="fa-solid fa-share" data-bs-toggle="tooltip" data-bs-placement="bottom"
                    ...
                    {{-- <li><a href="{{route('learner.expire',$value->id)}}" title="Custom Seat Expire"><i
                        class="fas fa-calendar"></i></a></li> --}}

                    <!-- To Handle Paylater & Pending Amount Icon -->
                    @if(paylater($learner_detail_id) || pending_amt($learner_detail_id))
                    <li><a href="{{ route('learner.pending.payment', ['id' => $transaction->id]) }}"
                            data-bs-toggle="tooltip" data-bs-placement="bottom" title=""
                            data-original-title="Send Email Reminders" class="payment-learner w-auto px-2">Pay Due</a>
                    </li>
                    @endif

                    @if(overdue($learner_id, learnerTransaction($learner_id, $learner_detail_id)->pending_amount) )
                    <li>
                        <a class="" target="_blank" data-bs-placement="bottom" data-bs-toggle="tooltip"
                            data-bs-title="Send Pending Payment Reminder" href="https://wa.me/+91{{ $value->mobile }}?text={{ rawurlencode(
                                    'Dear ' . $value->name . "\n\n" .
                    ...
                    @if($planStatus['diff_in_days'] <= 5 && $planStatus['diff_extend_day']>= 0 &&
                        !alreadyRenewed($learner_id) && $value->frozen_status != 1)
                        @can('has-permission','Renew Seat')
                        <li>
                            <a class="renew_extend w-auto px-2" data-seat_no="{{$value->seat_no}}"
                                data-user="{{$learner_id}}" data-end_date="{{$value->plan_end_date}}"
                                data-learner_detail="{{$learner_detail_id}}">Renew</a>
                        ...
                        @if($planStatus['diff_in_days'] <= 5 && $planStatus['diff_extend_day']>= 0 )
                            ...
                            target="_blank" data-id="11" onclick="incrementMessageCount({{ $value->id }}, 'email')"
                            class="message" data-bs-toggle="tooltip" data-bs-placement="bottom" title=""
                        ...
                        <form action="{{ route('generateIdCard') }}" method="POST" enctype="multipart/form-data">
                            ...
                            @if($planStatus['diff_in_days'] <= 5 && $planStatus['diff_extend_day']>= 0 )
                                ...
                                <li><a href="{{route('learners.reactive',$value->id)}}" data-bs-placement="bottom"
                                        data-bs-toggle="tooltip" data-bs-title="Reactivate Learner"><i
                                            class="fa-solid fa-arrows-rotate"></i></a></li>
                                @endif
                                @endcan

                                @can('has-permission', 'Add Misllaneous Payment')
                                <li><a href="{{route('learner.other.payment',$learner_detail_id)}}"
                                        data-bs-placement="bottom" data-bs-toggle="tooltip"
                                        data-bs-title="Other Payment" class="payment-learner"><i
                                            class="fa-solid fa-money-bill"></i></a></li>
                                @endcan
                                @can('has-permission', 'Gift Days')
                                @if(!in_array('33', toggleHideField()) && $value->frozen_status != 1)
                                <li><a href="javascript:;" class="giftDaysBtn" data-learner_id="{{$learner_id}}"
                                        data-bs-placement="bottom" data-bs-toggle="tooltip" data-bs-title="Gift Days"><i
                                            class="fa-solid fa-gift"></i></a></li>
                                @endif
                                @endcan
                                ...
                                <li><a href="{{route('learners.show',$value->id)}}" data-bs-placement="bottom"
                                        data-bs-toggle="tooltip" data-bs-title="View Seat Booking Full Details"><i
                        ...
                        <li><a href="#" data-id="{{$learner_id}}" data-learnerDetail="{{ $learner_detail_id }}"
                                ...
                                <li>
                                    <a target="_blank"
                                        ...
                                        <i class="fa-solid fa-receipt" data-bs-toggle="tooltip"
            ...
            <div class="seat-informarion">
                ...
                <a href="{{ asset($value->profile_picture) }}" class="view-image learner-list-profile-photo"
                ...
                <div class="information">
                    ...
                    <span>UID : <a href="{{route('learners.show',$value->id)}}">{{$value->learner_no}}</a> &nbsp; |
                        &nbsp; M : <a href="tel:+91-{{$value->mobile}}">+91-{{$value->mobile}}</a> </span>
                    <span class="d-block">E: <a href="mailto:{{$value->email}}"> {!! $value->email ? $value->email : '<i
                                class="fa-solid fa-times text-danger"></i> Email ID Not Available' !!} </a></span>
                </div>
            </div>
            <div class="plan-info">
                <ul>
                    ...
                    <li>
                        ...
                        <div class="d-flex g-1">
                            @if(paylater($learner_detail_id) &&
                            learnerTransaction($learner_id,$learner_detail_id)->pending_amount!=0)
                            <a href="{{ route('learner.pending.payment', ['id' => $transaction->id]) }}"
                                class="text-danger d-block">
                                <span class="extended" data-bs-title="Popover title"
                                    data-bs-content="And here’s some amazing content. It’s very engaging. Right?">
                                    PayLater
                                    {{ rtrim(rtrim(number_format(   (learnerTransaction($learner_id, $learner_detail_id))->pending_amount, 2, '.', ''), '0'), '.') }}
                                </span>
                            </a>
                            @elseif(!empty(learnerTransaction($learner_id,$learner_detail_id)->pending_amount) &&
                            learnerTransaction($learner_id,$learner_detail_id)->pending_amount==0)
                            <span class="payment" data-bs-title="Popover title"
                                data-bs-content="And here’s some amazing content. It’s very engaging. Right?">Fully
                                Paid</span>

                            <form action="{{ route('fee.generateReceipt') }}" method="POST"
                                enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="learner_id" value="{{$learner_id}}">
                                <input type="hidden" name="id"
                                    value="{{ learnerTransaction($learner_id,$learner_detail_id)->id ?? 'NA'}}">
                            ...
                            <a href="{{ route('learner.pending.payment', ['id' => $transaction->id]) }}"
                                class="text-danger d-block">
                                @if( !empty($due_date) && overdue($learner_id, learnerTransaction($learner_id,
                                $learner_detail_id)->pending_amount))
                                <span class="extended" data-bs-title="Popover title"
                                    data-bs-content="And here’s some amazing content. It’s very engaging. Right?">Due
                                    {{ rtrim(rtrim(number_format(optional(learnerTransaction($learner_id, $learner_detail_id))->pending_amount, 2, '.', ''), '0'), '.') }}
                                    ({{date('j M', strtotime($due_date))}})</span>
                                @else
                                <span class="extended" data-bs-title="Popover title"
                                    data-bs-content="And here’s some amazing content. It’s very engaging. Right?">
                                    Due
                                    {{ rtrim(rtrim(number_format((learnerTransaction($learner_id, $learner_detail_id))->pending_amount, 2, '.', ''), '0'), '.') }}
                                    ({{ !empty($due_date) ? date('j M', strtotime($due_date)) : ''}})
...
<ul class="paginations mt-4">
    {{-- Prev --}}
    <li>
        <a href="{{ $learners->onFirstPage() ? '#' : $learners->previousPageUrl() }}"
            class="w-auto px-3 text-muted">Prev</a>
    </li>
    ...
    @for ($i = max(1, $learners->currentPage() - 2); $i <= min($learners->lastPage(), $learners->currentPage() + 2);
        $i++)
        <li>
        ...
        @if ($learners->currentPage() < $learners->lastPage() - 2)
            ...
            {{-- Next --}}
            <li>
                <a href="{{ $learners->hasMorePages() ? $learners->nextPageUrl() : '#' }}"
                    class="w-auto px-3 text-muted">Next</a>
            </li>
</ul>
@endif

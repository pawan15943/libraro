...
class LearnerService
{
    ...
    public function getAvailableSeats()
    {
        ...
        $firstRecord = Hour::where('branch_id', $branchId)->first(); 
        ...
        // Step 1: Get used hours for each seat in the active branch for active plans
        $usedSeats = LearnerDetail::where('branch_id', $branchId)
            ->where('status', 1)
            ->whereDate('plan_end_date', '>=', \Carbon\Carbon::today())
    ...
    public function getAvailableSeatsPlantype()
    {
        ...
        $dayType8PlanTypeId = $planTypes->firstWhere('day_type_id', 8)?->id ?? 0;

        $allBookings = Learner::leftJoin('learner_detail', 'learner_detail.learner_id', '=', 'learners.id')
            ...
            ->get(['learner_detail.seat_no', 'learner_detail.plan_type_id', 'plan_types.start_time', 'plan_types.end_time', 'plan_types.slot_hours', 'plan_types.day_type_id'])
            ->groupBy(fn ($row) => (int) $row->seat_no);

        $seatPlanTypes = [];

        for ($seatNo = 1; $seatNo <= $total_seats; $seatNo++) {
            $bookings = $allBookings->get($seatNo) ?? collect();
            $planTypesRemovals = [];
            $totalBookedHours = $bookings->sum('slot_hours');
            $nightseatBooked = $bookings->contains('day_type_id', 9);
            ...
            if ($totalBookedHours > 1 && $dayType8PlanTypeId) {
                $planTypesRemovals[] = $dayType8PlanTypeId;
            }

            if ($nightseatBooked) {
                $nightPlanTypeId = $bookings->firstWhere('day_type_id', 9)?->plan_type_id ?? 0;
                if ($nightPlanTypeId) {
                    $planTypesRemovals[] = $nightPlanTypeId;
                }
            }
    ...
    private function formatSeatLearner($detail, $transactions)
    {
        ...
        return [
            ...
            'freeze_date'=>$detail->freeze_start_date ?? '',
            'pending_amount' => (string) ($transaction->pending_amount ?? 0),
            'pending_payment_overdue' => (float) ($transaction->pending_amount ?? 0) > 0 && !empty($transaction->due_date) && $transaction->due_date < date('Y-m-d'),
            'due_date' => $transaction->due_date,

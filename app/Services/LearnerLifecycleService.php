...
class LearnerLifecycleService
{
    ...
    private function formatSubscriptionTransaction($transaction, int $index, float $carryForwardAmount, float $extraPaidAmount = 0, int $firstTransactionId = 0): array
    {
        ...
        return [
            ...
            'status' => $mainstatus ?? '',
            'status_badge' => $mainstatus ?? '',
            'seat_no' => (string) ($detail?->seat_no ?? ''),
            ...
            'subscription_download_receipt_link' => \Illuminate\Support\Facades\URL::signedRoute('receipt.signed', ['transactionId' => $transaction->id]),
